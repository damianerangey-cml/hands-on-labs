...
GPU SIZING. Because stages are queued rather than run wherever the agent happens
to live, each one asks for the slice it actually needs. That is the point of
putting them on a platform.
...
TAO_IMAGE = os.environ.get("TAO_PYT_IMAGE", "nvcr.io/nvstaging/tao/tao-toolkit-pyt")
...
# Queue per stage = GPU per stage. Diffusion wants a whole card; k-NN mining
# over SigLIP embeddings does not.
QUEUE_FULL = os.environ.get("DEFT_QUEUE_FULL", "1XGPU")
QUEUE_HALF = os.environ.get("DEFT_QUEUE_HALF", "0.5XGPU")
QUEUE_SMALL = os.environ.get("DEFT_QUEUE_SMALL", "0.25XGPU")
...
# --------------------------------------------------------------------------
def train(spec, iteration, queue=QUEUE_HALF, workspace=WORKSPACE, wait=True):
...
def anomalygen_prep(iteration, dataset_dir, num_sdg, run_dir,
                    queue=QUEUE_SMALL, workspace=WORKSPACE, wait=True):
